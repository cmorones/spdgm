...
if ($tipo==0) {
	$conciliado = "No conciliados";
		 $sql = "SELECT  fecha_ingreso, folio, no_contrarecibo, importe, validado FROM 
  		     base_cap where validado=$tipo and (fecha_ingreso BETWEEN '$fecha1' AND '$fecha2')";
}elseif ($tipo==1) {
	...
	$conciliado = "";
		 $sql = "SELECT  fecha_ingreso, folio, no_contrarecibo, importe, validado FROM 
  		     base_cap where (fecha_ingreso BETWEEN '$fecha1' AND '$fecha2')";
}
...
<div class='span9'>
<h3 aling='center'>Reporte  $conciliado del $fecha1 al $fecha2 </h3><br>
