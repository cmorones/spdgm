...
<i class="icon-reorder"></i>
 <div class="portlet-title">Informe de Gastos General
 </div>
<div class="form">
...
if(isset($fecha1,$fecha2,$partida)){

//echo $partida;
//echo $fecha2;

	if (($fecha1 != '') && ($fecha1 != '') && ($partida != 0) && ($id_bandera != 0)) {

	/*$resultado = BaseCap::model()->findAll((array(
  //  'condition'=>'bandera=1',
   //'condition'=>"bandera=1 and subprog=$subprog",
    'condition'=>"bandera=1 and partida=$partida and (fecha_ingreso BETWEEN '$fecha1' AND '$fecha2')",
    'order'=>'partida,subprog'
	)));*/

	$criteria = new CDbCriteria();
	$criteria->select = 'id, area, folio, proveedor, concepto, detalle,  fecha_contrarecibo, no_contrarecibo, subprog, partida, fecha_ingreso, importe';
	$criteria->condition = 'bandera=:bandera AND
							partida=:partida 
							';
	...
	$criteria->addBetweenCondition("fecha_ingreso",$fecha1,$fecha2,"AND");							
	$criteria->order = "partida,subprog,fecha_ingreso";
  ...
  //  'condition'=>'bandera=1',
   //'condition'=>"bandera=1 and subprog=$subprog",
	...
	$criteria = new CDbCriteria();
	$criteria->select = 'id, area, folio, proveedor, concepto, detalle,  fecha_contrarecibo, no_contrarecibo, subprog, partida, fecha_ingreso, importe';
	$criteria->condition = 'bandera=:bandera  
							';
	$criteria->params = array(
							':bandera'=>$id_bandera
...
$json =array();
$titulo = "Informe General partidas por bandera del $fecha1 al $fecha2";
...
foreach ($resultado as $key => $row) {
	...
	//	$json["informe"]['grantotal'] = $json["informe"]['grantotal'] + $json["informe"]['partida'][$row["partida"]]['id'][$row["id"]]['importe'];
