...
*/
	...
	foreach ($resultado as $key => $row) {
		...
		$sql = "SELECT SUM(saldo_actual) as suma from ing_ext where id_bandera=$row[id]"; 
		$saldo = Yii::app()->db->createCommand($sql)->queryRow();

		//Calculo de depositos enero
		$sql = "SELECT SUM(importe) as deposito from base_ing where bandera=$row[id] and fecha_ingreso BETWEEN '2014-01-01' AND '2014-01-31'"; 
		...
		$sql = "SELECT SUM(importe) as gastos from base_cap where bandera=$row[id] and fecha_ingreso BETWEEN '2014-01-01' AND '2014-01-31'"; 
		...
		$sql = "SELECT SUM(importe) as deposito from base_ing where bandera=$row[id] and fecha_ingreso BETWEEN '2014-02-01' AND '2014-02-28'"; 
		...
		$sql = "SELECT SUM(importe) as gastos from base_cap where bandera=$row[id] and fecha_ingreso BETWEEN '2014-02-01' AND '2014-02-28'"; 
		...
		$sql = "SELECT SUM(importe) as deposito from base_ing where bandera=$row[id] and fecha_ingreso BETWEEN '2014-03-01' AND '2014-03-31'"; 
	   ...
	   //Calculo de GASTOS marzo
		$sql = "SELECT SUM(importe) as gastos from base_cap where bandera=$row[id] and fecha_ingreso BETWEEN '2014-03-01' AND '2014-03-31'"; 
		...
		$sql = "SELECT SUM(importe) as deposito from base_ing where bandera=$row[id] and fecha_ingreso BETWEEN '2014-04-01' AND '2014-04-30'"; 
	   ...
	   //Calculo de GASTOS abril
		$sql = "SELECT SUM(importe) as gastos from base_cap where bandera=$row[id] and fecha_ingreso BETWEEN '2014-04-01' AND '2014-04-30'"; 
		...
		$sql = "SELECT SUM(importe) as deposito from base_ing where bandera=$row[id] and fecha_ingreso BETWEEN '2014-05-01' AND '2014-05-31'"; 
	   ...
	   //Calculo de GASTOS abril
		$sql = "SELECT SUM(importe) as gastos from base_cap where bandera=$row[id] and fecha_ingreso BETWEEN '2014-05-01' AND '2014-05-31'"; 
		...
		$sql = "SELECT SUM(importe) as deposito from base_ing where bandera=$row[id] and fecha_ingreso BETWEEN '2014-06-01' AND '2014-06-30'"; 
	   ...
	   //Calculo de GASTOS abril
		$sql = "SELECT SUM(importe) as gastos from base_cap where bandera=$row[id] and fecha_ingreso BETWEEN '2014-06-01' AND '2014-06-30'"; 
        ...
        $depositosmeses = $saldo['suma'] + $deposito + $depositofeb + $depositomar + $depositoabr + $depositomay + $depositojun;
        $gastosmeses = $gastos + $gastosfeb + $gastosmar + $gastosabr + $gastosmay + $gastosjun;
	   ...
	   //SUMA DE DEPOSITOS JUNIO
        ...
        $json["informe"]['ingresotot'] = $json["informe"]['SaldoInicialTotal'] + $json["informe"]['ingenet'] + $json["informe"]['ingfebt'] + $json["informe"]['ingmart'] + $json["informe"]['ingabrt'] + $json["informe"]['ingmayt'];
        $json["informe"]['gastotot'] = $json["informe"]['gastoenet'] + $json["informe"]['gastofebt'] + $json["informe"]['gastomart'] + $json["informe"]['gastoabrt'] + $json["informe"]['gastomayt'];
        $json["informe"]['disponiblet'] = $json["informe"]['ingresotot'] - $json["informe"]['gastotot'] ;


	     //SUMA DE DEPOSITOS TOTAL
	//	$json["informe"]['ingfebt'] = $json["informe"]['ingfebt'] + $json["informe"]['cuenta'][$row["nombre"]]['depositoFeb'];
	   //SUMA DE DEPOSITOS TOTAL
	//	$json["informe"]['ingresotot'] = $json["informe"]['SaldoInicialTotal'] + $json["informe"]['ingenet'] ;	  
	//	$json["informe"]['gastotot'] = $json["informe"]['gastoenet'] + $json["informe"]['gastofebt'] ;	  
	//	$json["informe"]['disponiblet'] = $json["informe"]['SaldoInicialTotal'] + $json["informe"]['ingenet'] ;	  
	...
	/*$resultado = BaseIng::model()->findAll((array(
    'condition'=>"bandera=$row[id]",
 //   'condition'=>"fecha_ingreso BETWEEN '$fecha1' AND '$fecha2'",
    'order'=>'nombre'
	)));
	foreach ($resultado as $key => $rowing) {


	}
	$resultado2 = BaseIng::model()->findAll((array(
    'condition'=>"bandera=$row[id]",
    'condition'=>"fecha_ingreso BETWEEN '$fecha1' AND '$fecha2'",
    'order'=>'bandera'
	)));
   ...
   /*if(is_array($resultado2)){
	foreach ($resultado2 as $key => $rowing) {

		$fecha =$rowing["fecha_ingreso"]; 
		$date = strtotime("$fecha");

		


		if( !isset($json["informe"]['cuenta'][$row["nombre"]]["eneing"][$rowing["fecha_ingreso"]]) && date("m", $date) =='1'){ 
			$json["informe"]['cuenta'][$row["nombre"]]["eneing"][$rowing["fecha_ingreso"]] = array(
				'importe' => $rowing["importe"], 
			);
	}
	}
	$json["informe"]['cuenta'][$row["nombre"]]["eneing"][$rowing["fecha_ingreso"]]["importe"]=0;
}
	/*$resultado = BaseCap::model()->findAll((array(
    'condition'=>"bandera=$row[id]",
    'condition'=>"fecha_ingreso BETWEEN '$fecha1' AND '$fecha2'",
...
/*	foreach ($resultado as $key => $rowgas) {

		$fecha =$rowing["fecha_ingreso"]; 
		$date = strtotime("$fecha");

		if( !isset($json["informe"]['cuenta'][$row["nombre"]]["enegas"][$rowgas["fecha_ingreso"]]) && date("m", $date) =='1'){ 
			$json["informe"]['cuenta'][$row["nombre"]]["enegas"][$rowgas["fecha_ingreso"]] = array(
				'importe' => $rowgas["importe"], 
			);
	}
}*/
		 //Enero
		/*if( !isset($json["informe"]['cuentas'][$row["nombre"]]["ene"][$row["fecha_ingreso"]]) && date("m", $date) =='1'){ 
			$json["informe"]['cuentas'][$row["nombre"]]["ene"][$row["fecha_ingreso"]] = array(
				'importe' => $row["importe"], 
			);
		
		$json["informe"]['cuentas'][$row["nombre"]]["totalene"] = $json["informe"]['partida'][$row["partida"]]["totalene"]  + $json["informe"]['partida'][$row["partida"]]["ene"][$row["fecha_ingreso"]]['importe'];
		$json["informe"]['enet'] = $json["informe"]['enet'] + $json["informe"]['partida'][$row["partida"]]["ene"][$row["fecha_ingreso"]]['importe'];
...
$sep = $sep + $base['importe'];
		...
		$presupuesto = Yii::app()->db->createCommand($sql)->queryRow();
			...
			$json["informe"]['partida'][$row] = array(
			  ...
			  'dic' => array(),
			//  'totalenero' => 0,
              'totalejercido' => $ene + $feb + $mar + $abr + $may + $jun + $jul + $ago + $sep,
              'presupuesto'=> $presupuesto['suma'],
             // 'totalejercido' => 0,
              'disponible'=> 0,
				);
		...
		$json["informe"]['ejercidot'] =  $json["informe"]['enet'] +  $json["informe"]['febt'] +  $json["informe"]['mart'] +  $json["informe"]['abrt'] +  $json["informe"]['mayt'] +  $json["informe"]['junt'] +  $json["informe"]['jult'] +  $json["informe"]['agot'] +  $json["informe"]['sept'] ;
		$json["informe"]['disponiblet'] = $json["informe"]['presupuestot'] - $json["informe"]['ejercidot'];
...
public function actionSubprog($fecha1,$fecha2,$subprog,$id_periodo,$id_trim) {
	$this->layout=false;
...
$url = "http://localhost/spdgm/index.php/api/ptop?id_subprog=$id_subprog&id_partida=$id_partida";
//$url = $baseUrl;
$data = file_get_contents($url);
$model= CJSON::decode($data);
...
$titulo = "Informe de depositos y gastos por ingresos extraordinarios";
