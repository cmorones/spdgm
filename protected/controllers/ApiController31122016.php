...
public function actionSubprog($fecha1,$fecha2,$subprog,$id_partida,$id_periodo,$id_trim,$id_area) {
	$this->layout=false;
...
$filtroint2 ="";
//echo $fecha1;
$filtroint2 .="id_periodo = $id_periodo  and area<>12 and partida=$row[partida] and partida<>211 and partida<>331 and partida<>612 and (fecha_ingreso BETWEEN '$fecha1' and '$fecha2') AND ";

if($subprog !=0){
	$filtroint2 .="subprog =$subprog AND ";
}
...
//$cmd->getText();
	...
	$json["informe"]['partida'][$row["partida"]]["totalene"] = 0;
	$json["informe"]['partida'][$row["partida"]]["totalfeb"] = 0;
...
//echo "<br>" .$contador++;
	...
	/*$sql = "SELECT SUM(importe) as suma from base_cap where partida=$row[partida]  and (fecha_ingreso BETWEEN '2016-01-01' and '2016-01-31') "; 
		...
		/*$sql = "SELECT SUM(importe) as suma from base_cap where partida=$row[partida] $filtro2 and (fecha_ingreso BETWEEN '2016-02-01' and '2016-02-29') "; 
		...
		if($gasto_feb["suma"] > 0){
		 ...
		 //Enero
		/*if( !isset($json["informe"]['partida'][$row["partida"]]["ene"][$row["id"]])){ 
			$json["informe"]['partida'][$row["partida"]]["ene"]= array(
				'importe' => $gasto_ene["suma"], 
			);
		
		//$json["informe"]['partida'][$row["partida"]]["totalene"] = $json["informe"]['partida'][$row["partida"]]["totalene"]  + $json["informe"]['partida'][$row["partida"]]["ene"]['importe'];
		$json["informe"]['enet'] = $json["informe"]['enet'] + $json["informe"]['partida'][$row["partida"]]["ene"]['importe'];
...
$report = "ReporteGeneralDetalle-". date("d/m/y H:i:s").".pdf";
...
background-position-x: 172px;
    ...
    table {
        font-size: 12px;      padding: 10px 40px 40px 40px;     width: 800px; text-align: center;
        border-collapse: collapse;
        margin:0;
    }
    thead {
...
$html .='
<header aling="center">Informe de cheques $pagado</header>
<body>
