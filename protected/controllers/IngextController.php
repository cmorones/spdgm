...
class IngextController extends Controller
{
	...
	public function actionCreate($id)
	{
		$model=new Ingext;
		 ...
		 $resultBanderas = Banderas::model()->findAll('status=1 and ingext=1',array('order'=>'id'));
        $banderas = array();
		...
		if(isset($_POST['Ingext']))
		{
			...
			$model->id_ejercicio=$id;
			$model->estado=1;
			$model->fecha_ing=date("Y-m-d H:i:s");
			if($model->save())
				$this->redirect(array('admin','id'=>$model->id_ejercicio));
		}
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='ingext-form')
		{
